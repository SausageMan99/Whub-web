...
def is_safe_layout_retry_report(report: dict) -> bool:
    ...
    hard_blocks_contact = bool(report.get("contact_hits"))
    hard_blocks_identity = bool(
        report.get("identity_hits")
        or report.get("candidate_identity_term_exposed")
        or report.get("full_name")
        or codes.intersection({"candidate_identity_term_exposed", "full_name_display"})
    )
    return (
        bool(codes.intersection({
            "page_too_dense",
            "last_page_sparse",
            "page_too_sparse",
            "bad_page_break",
        ...
        and not hard_blocks_contact
        and not hard_blocks_identity
